...
def changeFooter():
    ...
    print("seima")
    myCursor = myConnection.cursor()
    myCursor.execute("UPDATE footer SET contact_footer = '" + data["contact_footer"] + "', copyright = '"+ data["copyright"] + "' where 1=1")
    myConnection.commit()
    myConnection.close()
    return jsonify(True)
...
def changeArticle():
    ...
    print(id)
    myCursor = myConnection.cursor()
    myCursor.execute("UPDATE articles SET img = '"+img+"', title = '"+title+"', intro='"+intro+"', date='"+date+"', category='"+category+"', content = '"+content+"'   where id = "+id)
    myConnection.commit()
    myConnection.close()
    return jsonify(True)
